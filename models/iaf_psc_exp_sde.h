...
#ifndef IAF_PSC_EXP_SDE_H
#define IAF_PSC_EXP_SDE_H
...
iaf_psc_exp_sde is an implementation of a leaky integrate-and-fire model
...
optimization levels. A future version of iaf_psc_exp_sde will probably
...
iaf_psc_exp_sde can handle current input in two ways: Current input
...
iaf_cond_exp, iaf_psc_exp_sde_ps

EndUserDocs */

class iaf_psc_exp_sde : public ArchivingNode
{

public:
  iaf_psc_exp_sde();
  iaf_psc_exp_sde( const iaf_psc_exp_sde& );
...
private:
  ...
  friend class RecordablesMap< iaf_psc_exp_sde >;
  friend class UniversalDataLogger< iaf_psc_exp_sde >;
  ...
  struct Buffers_
  {
    Buffers_( iaf_psc_exp_sde& );
    Buffers_( const Buffers_&, iaf_psc_exp_sde& );
    ...
    UniversalDataLogger< iaf_psc_exp_sde > logger_;
  };
  ...
  struct Variables_
  {
    ...
    */
    ...
    // not necessary for this model: time evolution operator
    // double P20_;
    // double P11ex_;
    // double P11in_;
    // double P21ex_;
  ...
  static RecordablesMap< iaf_psc_exp_sde > recordablesMap_;
};


inline port
nest::iaf_psc_exp_sde::send_test_event( Node& target, rport receptor_type, synindex, bool )
{
  SpikeEvent e;
  e.set_sender( *this );
...
inline port
iaf_psc_exp_sde::handles_test_event( SpikeEvent&, rport receptor_type )
{
  if ( receptor_type != 0 )
  {
...
inline port
iaf_psc_exp_sde::handles_test_event( CurrentEvent&, rport receptor_type )
{
  if ( receptor_type == 0 )
  {
    return 0;
...
inline port
iaf_psc_exp_sde::handles_test_event( DataLoggingRequest& dlr, rport receptor_type )
{
  if ( receptor_type != 0 )
  {
    throw UnknownReceptorType( receptor_type, get_name() );
...
inline void
iaf_psc_exp_sde::get_status( DictionaryDatum& d ) const
{
  P_.get( d );
  S_.get( d, P_ );
...
inline void
iaf_psc_exp_sde::set_status( const DictionaryDatum& d )
{
