...
nest::RecordablesMap< nest::iaf_psc_exp_sde > nest::iaf_psc_exp_sde::recordablesMap_;
...
template <>
void
RecordablesMap< iaf_psc_exp_sde >::create()
{
  // use standard names whereever you can for consistency!
  insert_( names::V_m, &iaf_psc_exp_sde::get_V_m_ );
  insert_( names::I_syn_ex, &iaf_psc_exp_sde::get_I_syn_ex_ );
  insert_( names::I_syn_in, &iaf_psc_exp_sde::get_I_syn_in_ );
}
}
...
nest::iaf_psc_exp_sde::Parameters_::Parameters_()
  : Tau_( 10.0 )             // in ms
  , C_( 250.0 )              // in pF
  , t_ref_( 2.0 )            // in ms
  , E_L_( -70.0 )            // in mV
...
nest::iaf_psc_exp_sde::State_::State_()
  : i_0_( 0.0 )
  , i_1_( 0.0 )
  , i_syn_ex_( 0.0 )
  , i_syn_in_( 0.0 )
...
void
nest::iaf_psc_exp_sde::Parameters_::get( DictionaryDatum& d ) const
{
...
double
nest::iaf_psc_exp_sde::Parameters_::set( const DictionaryDatum& d, Node* node )
{
...
void
nest::iaf_psc_exp_sde::State_::get( DictionaryDatum& d, const Parameters_& p ) const
{
  def< double >( d, names::V_m, V_m_ + p.E_L_ ); // Membrane potential
}

void
nest::iaf_psc_exp_sde::State_::set( const DictionaryDatum& d, const Parameters_& p, double delta_EL, Node* node )
{
...
nest::iaf_psc_exp_sde::Buffers_::Buffers_( iaf_psc_exp_sde& n )
  : logger_( n )
{
}

nest::iaf_psc_exp_sde::Buffers_::Buffers_( const Buffers_&, iaf_psc_exp_sde& n )
  : logger_( n )
{
}
...
nest::iaf_psc_exp_sde::iaf_psc_exp_sde()
  : ArchivingNode()
  , P_()
  , S_()
  , B_( *this )
...
nest::iaf_psc_exp_sde::iaf_psc_exp_sde( const iaf_psc_exp_sde& n )
  : ArchivingNode( n )
  , P_( n.P_ )
  , S_( n.S_ )
  , B_( n.B_, *this )
...
void
nest::iaf_psc_exp_sde::init_buffers_()
{
...
void
nest::iaf_psc_exp_sde::calibrate()
{
  ...
  // a double in ms. The grid based iaf_psc_exp_sde can only handle refractory
...
void
nest::iaf_psc_exp_sde::update( const Time& origin, const long from, const long to )
{
  ...
  {
    // PSCs at the beginning of the time step
    double i_syn_ex_1 = S_.i_syn_ex_;
    double i_syn_in_1 = S_.i_syn_in_; 
    // exponential decaying PSCs at the end of time step
    S_.i_syn_ex_ *= std::exp( -h / P_.tau_ex_ );
    S_.i_syn_in_ *= std::exp( -h / P_.tau_in_ );

    if ( S_.r_ref_ == 0 ) // neuron not refractory, so evolve V
    {
      // Runge-Kutta coefficients
      double K1 = - S_.V_m_ * h / P_.Tau_
	+ ( i_syn_ex_1 + i_syn_in_1 + P_.I_e_ + S_.i_0_ ) * h / P_.C_;
      double K2 = - ( S_.V_m_ + K1 ) * h / P_.Tau_
	+ ( S_.i_syn_ex_ + S_.i_syn_in_ + P_.I_e_ + S_.i_0_ ) * h / P_.C_;
      S_.V_m_ = S_.V_m_ + ( K1 + K2 ) / 2.0;
    }
    else
    {
    ...
    S_.i_syn_ex_ += ( 1. - std::exp( -h / P_.tau_ex_ ) ) * S_.i_1_;
...
void
nest::iaf_psc_exp_sde::handle( SpikeEvent& e )
{
...
void
nest::iaf_psc_exp_sde::handle( CurrentEvent& e )
{
  assert( e.get_delay_steps() > 0 );
...
void
nest::iaf_psc_exp_sde::handle( DataLoggingRequest& e )
{
  B_.logger_.handle( e );
}
